...
def parse_args(args=None):
    ...
    parser.add_argument('-g', '--gamma', default=12.0,
    ...
    parser.add_argument('--geo', default='cylinder', type=str, choices=['cylinder'],
                        help='the reasoning model, cylinder for cylE')
    parser.add_argument('-cenr', '--center_reg', default=0.02, type=float,
                        help='center_reg for cylE, center_reg balances the dist in and dist out')
    parser.add_argument('-projm', '--proj_mode', default="(1600,2)", type=str,
                        help='(hidden_dim,num_layer) for relational projection')
...
def main(args):
    ...
    args.save_path = os.path.join(
        prefix, args.data_path.split('/')[-1], args.geo)
    if args.geo in ['cylinder']:
        tmp_str = "g-{}-mode_proj-({}.{})-dim-{}".format(
            args.gamma, args.proj_mode[1:5], args.proj_mode[6:7], args.hidden_dim)
    ...
    model = KGReasoning(
        ...
        geo=args.geo,
        use_cuda=args.cuda,
        proj_mode=eval_tuple(args.proj_mode),
    ...
    step = init_step
    if args.geo == 'cylinder':
        logging.info('cylinder mode (proj_mode, cylinder_mode) = (%s, %s)' %
                     (args.proj_mode, args.center_reg))
